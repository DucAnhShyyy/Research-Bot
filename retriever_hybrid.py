...
class HybridRetriever:
    """
    A retriever focusing on Dense vector search.
    (BM25 disabled temporarily to fix 400 Bad Request error due to missing Sparse Vector config)
    """
    ...
    def _parse_hit(self, hit):
        """
        Convert Qdrant search result into a dict.
        """
        # Case 1: Object style (New Qdrant)
        if hasattr(hit, "payload"):
            return {
                "id": hit.id,
                "score": hit.score,
        ...
        # Case 2: Tuple style (Old Qdrant)
        if isinstance(hit, tuple):
            h = list(hit) + [None] * (4 - len(hit))
            doc_id, score, payload, vector = h[:4]
            return {
                "id": doc_id,
                "score": score,
    ...
    def dense_search(self, query: str, k=10):
        try:
            ...
            hits = self.client.query_points(
                collection_name=self.collection,
                query=q_vec,  # Truyền vector vào đây (đúng chuẩn)
                limit=k,
            )
            return [self._parse_hit(h).get("points", h) if isinstance(h, dict) else self._parse_hit(h) for h in hits.points] if hasattr(hits, 'points') else [self._parse_hit(h) for h in hits]
            
        except Exception as e:
            logger.error(f"Dense search failed: {e}")
            # Fallback nếu query_points kiểu mới gặp lỗi version, thử search cũ
            try:
    ...
    def merge_and_rerank(self, query: str, top_k=5):
        """
        Modified to use ONLY Dense Search to prevent crash.
        """
        # Bỏ qua BM25 vì collection chưa cấu hình Sparse Vector
        # Chạy Dense Search thuần túy
        results = self.dense_search(query, k=top_k)
        
        # Sắp xếp lại (dù Qdrant đã sort rồi, nhưng để đảm bảo logic)
        results = sorted(results, key=lambda x: x["score"], reverse=True)
